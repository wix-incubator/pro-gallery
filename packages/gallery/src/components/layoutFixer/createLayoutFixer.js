...
export const createLayoutFixer = (mediaUrlFixer) => {
  const getQueryParams = () => {
    ...
    const keys = [
      'useLayoutFixer',
      'disableInlineStyles',
      'dontRemoveHiddenItems',
      'disableSavedBlueprint',
      'useCssTag',
      'dontSetHighResImage',
      'enableAfterMount',
      'disableAfterHydrate',
    ...
    class LayoutFixerElement extends HTMLElement {
      ...
      connectedCallback() {
        ...
        if (!window.layoutFixer[this.domId]) {
          ...
          window.layoutFixer[this.domId].mounted &&
          !canUse.enableAfterMount
        ) {
          return;
        } else if (
          window.layoutFixer[this.domId].hydrated &&
          canUse.disableAfterHydrate
        ) {
          return;
        }
        canUse.useLayoutFixer &&
        ...
        this.items = JSON.parse(this.getAttribute('items')).map(
        ...
        if (
          ...
          const itemContainers = this.parent.querySelectorAll(
            '.gallery-item-container'
          );
          const itemWrappers = this.parent.querySelectorAll(
            '.gallery-item-wrapper'
          );
          const itemHighResImage = this.parent.querySelectorAll(
            '[data-hook=gallery-item-image-img]'
          );

          if (itemWrappers.length > 0 && itemContainers.length > 0) {
            if (
              !canUse.dontSetHighResImage &&
              typeof mediaUrlFixer === 'function'
            ) {
                ...
                if (mediaUrl && typeof mediaUrl === 'string') {
                  ...
                  if (src) {
                    !idx &&
                      canUse.useLayoutFixer &&
                      console.log(
                        '[LAYOUT FIXER] set first Image src Style',
                        idx,
                        src
                      );
            ...
            if (canUse.useCssTag) {
              ...
              itemContainers.forEach((element, idx) => {
                ...
                !idx &&
                  canUse.useLayoutFixer &&
                  console.log(
                    '[LAYOUT FIXER] set first Container CSS',
                    idx,
                    getItemContainerStyle(
                      this.layout.items[idx],
                      this.styleParams
                    )
                  );
                cssStr += createCssStr(
              ...
              itemWrappers.forEach((element, idx) => {
                ...
                !idx &&
                  canUse.useLayoutFixer &&
                  console.log(
                    '[LAYOUT FIXER] set first Wrapper CSS',
                    idx,
                    getItemWrapperStyle(
                      this.layout.items[idx],
                      this.styleParams
                ...
                cssStr += createCssStr(
                  element,
                  getItemWrapperStyle(this.layout.items[idx], this.styleParams)
                );
              });
            ...
            if (!canUse.avoidInlineStyles) {
              itemContainers.forEach((element, idx) => {
                if (idx >= visibleItems) {
                  canUse.useLayoutFixer &&
                    console.log(
                      '[LAYOUT FIXER] removed element from dom',
                      idx,
                      visibleItems
                    );
                  canUse.dontRemoveHiddenItems ||
                    element.parentElement.remove();
                } else {
                  !idx &&
                    canUse.useLayoutFixer &&
                    console.log(
                      '[LAYOUT FIXER] set first Container Style',
                      idx,
                      getItemContainerStyle(
                        this.layout.items[idx],
                        this.styleParams
              ...
              itemWrappers.forEach((element, idx) => {
                if (idx >= visibleItems) {
                  ...
                  !idx &&
                    canUse.useLayoutFixer &&
                    console.log(
                      '[LAYOUT FIXER] set first Wrapper Style',
                      idx,
                      getItemWrapperStyle(
                        this.layout.items[idx],
                        this.styleParams
            ...
            window.layoutFixer[this.domId].onMount = () => {
              ...
              removeStyleTag(
                this.parent,
                this.domId,
                canUse('removeCssOnMount'),
                canUse('removeCssWithDelay')
              );
            };
            ...
            canUse.useLayoutFixer &&
              console.log(
                '[LAYOUT FIXER] Cannot find elements',
                itemContainers,
                itemWrappers,
                this.parent
              );
