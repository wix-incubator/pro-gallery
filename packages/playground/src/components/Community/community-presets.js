      ...
      setItems(
        data.map((item) => ({
          ...
          options: item.url
            ...
            .reduce((options, [key, val]) => ({ ...options, [key]: formatValue(val) }), {}),
        }))
      );
    });
  }, []);
    ...
    .map((item) => (
      <Card
        ...
        actions={[
          <Button
            type="link"
            disabled={isDownvoted(item)}
            size="large"
            onClick={() => onClickLike(item, -1)}
            icon={<ArrowDownOutlined />}
          ></Button>,
          <Button
            type="link"
            disabled={isUpvoted(item)}
            size="large"
            onClick={() => onClickLike(item, 1)}
            icon={<ArrowUpOutlined />}
          ></Button>,
          <Button type="link" size="large">
            {isLiked(item) ? <StarFilled /> : <StarOutlined />} {item.likes}
          </Button>,
        ]}
        style={{
          margin: '20px 0',
      ...
      >
        <Card.Meta title={<a href={item.href}>{item.title}</a>} description={item.description} />
      </Card>
    ));
  // return (
  //   <List
