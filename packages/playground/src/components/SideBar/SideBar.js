...
function SideBar({ items, blueprintsManager, visible }) {
  const { gallerySettings, setGallerySettings, setOptions, options } =
  ...
  const dataSource = Object.entries(settingsManager)
    .filter(
      ([styleParam]) =>
        createSearchString(styleParam, searchTerm).indexOf(
          searchTerm.toLowerCase().replace(/ /g, "")
        ) >= 0
    )
    .sort(([styleParam, props], [styleParam2, props2]) =>
      props.title > props2.title ? 1 : -1
    )
  ...
  const changedOptions = Object.entries(flatOptions)
    .filter(
      ([styleParam, value]) =>
        styleParam !== "galleryLayout" &&
        isValidOption(styleParam, value, flatOptions)
    )
    .reduce(
      (obj, [styleParam, value]) => ({ ...obj, [styleParam]: value }),
      {}
    );
    ...
    <>
      ...
      >
        ...
        >
          <Input.Search
            size="large"
            placeholder="Search Style Param"
            enterButton
          />
        </AutoComplete>
        {searchResult && (
      ...
      <div className={s.controls}>
        ...
        >
          ...
          <Collapse.Panel header={SECTIONS.PRESET} key="1">
            <LayoutPicker
              selectedLayout={flatOptions.galleryLayout}
              onSelectLayout={(galleryLayout) =>
                setOptions("galleryLayout", galleryLayout)
              }
            />
            <Divider />
            <JsonEditor
        ...
        >
          <Collapse.Panel header="Media" key="media">
            <Form layout="vertical">
              <Form.Item
                ...
                help="Set to 0 for Infinite items"
              >
                <InputNumber
                  min={0}
                  max={100}
          ...
          <Collapse.Panel header="Title & Description" key="info">
            <Form layout="vertical">
              <Form.Item
                label="Custom Title"
                help="use the # symbol for the item index"
              >
                <Input
                  defaultValue={gallerySettings.customTitle || ""}
                  onChange={(e) =>
                    setGallerySettings({ customTitle: e.target.value })
                  }
                />
              </Form.Item>
              <Form.Item
              ...
              >
                <Input.TextArea
                  ...
                  rows="4"
                  onChange={(e) =>
                    setGallerySettings({ customDescription: e.target.value })
                  }
                />
              </Form.Item>
              ...
              >
                <Input
                  ...
                  onChange={(e) =>
                    setGallerySettings({ customAlt: e.target.value })
                  }
                />
              </Form.Item>
            </Form>
          ...
          <Collapse.Panel header="Simulators" key="simulators">
            <Form labelCol={{ span: 17 }} wrapperCol={{ span: 3 }}>
              <Form.Item label="Unknown dimension" labelAlign="left">
                <Switch
                  ...
                  onChange={(e) =>
                    setGallerySettings({ isUnknownDimensions: e })
                  }
                />
              </Form.Item>
              ...
              <Form.Item label="Use Custom Navigation Panel" labelAlign="left">
                <Switch
                  ...
                  onChange={(e) =>
                    setGallerySettings({ useCustomNavigationPanel: e })
                  }
                />
              </Form.Item>
              <Form.Item label="Use Server Blueprints" labelAlign="left">
                <Switch
                  ...
                  onChange={(e) =>
                    setGallerySettings({ shouldUseBlueprintsFromServer: e })
                  }
                />
              </Form.Item>
            ...
            <Form layout="vertical">
              <Form.Item label="View Mode">
                <Select
                  defaultValue={
                    gallerySettings.viewMode || GALLERY_CONSTS.viewMode.SITE
                  }
            ...
            <Collapse.Panel header="Lean Gallery" key="lean">
              <Form labelCol={{ span: 17 }} wrapperCol={{ span: 3 }}>
                <Form.Item label="Allow Lean Gallery" labelAlign="left">
                  <Switch
                    checked={!!flatOptions.allowLeanGallery}
                    onChange={(e) => setOptions("allowLeanGallery", !!e)}
                  />
                </Form.Item>
                {
          ...
          <>
            ...
            <div className={s.code}>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                href="https://github.com/wix/pro-gallery/issues"
              >
                report an issue
              </a>
            </div>
