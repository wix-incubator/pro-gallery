...
import { ArrowRightOutlined, BugOutlined, CloseOutlined, DeleteOutlined } from '@ant-design/icons';
...
function SideBar({ blueprintsManager, visible }) {
  const { gallerySettings, setGallerySettings, setOptions, options } = useGalleryContext(blueprintsManager);

  const [searchResult, setSearchResult] = useState('');
  const [searchTerm, setSearchTerm] = useState('');

  const _setOptions = throttle(setOptions, 1000);
  const createSearchString = (styleParam) => {
    let res = [styleParam];
  ...
  const dataSource = Object.entries(settingsManager)
    .filter(
      ([styleParam]) =>
        createSearchString(styleParam, searchTerm).indexOf(searchTerm.toLowerCase().replace(/ /g, '')) >= 0
    )
  ...
  const changedOptions = Object.entries(options)
    .filter(([styleParam, value]) => styleParam !== 'galleryLayout' && isValidOption(styleParam, value, options))
        ...
        >
          {didChangeStyleParams ? (
            <Collapse.Panel header={'* Changed Settings'} key="-1">
              <JsonEditor onChange={_setOptions} allOptions={options} options={changedOptions} showAllOptions={true} />
            </Collapse.Panel>
          ) : null}
          <Collapse.Panel header={SECTIONS.PRESET} key="1">
            <LayoutPicker
              selectedLayout={options[optionsMap.layoutParams.structure.galleryLayout]}
              onSelectLayout={(galleryLayout) =>
                setOptions(optionsMap.layoutParams.structure.galleryLayout, galleryLayout)
              }
            />
            <Divider />
            <JsonEditor
            ...
            <Form layout="vertical">
              <Form.Item label="Number of Items" help="Set to 0 for Infinite items">
                <InputNumber
                  min={0}
                  max={100}
                  defaultValue={gallerySettings.numberOfItems || 0}
                  onChange={(val) => setGallerySettings({ numberOfItems: val })}
                />
              </Form.Item>
              <Form.Item label="Initial Item">
                <InputNumber
                  min={0}
                  max={gallerySettings.numberOfItems || Infinity}
                  defaultValue={gallerySettings.initialIdx || 0}
                  onChange={(val) => setGallerySettings({ initialIdx: val })}
                />
              </Form.Item>
          ...
          <Collapse.Panel header="Title & Description" key="info">
            <Form layout="vertical">
              <Form.Item label="Custom Title" help="use the # symbol for the item index">
                <Input
                  defaultValue={gallerySettings.customTitle || ''}
                  onChange={(e) => setGallerySettings({ customTitle: e.target.value })}
                />
              </Form.Item>
              <Form.Item label="Custom Description" help="use the # symbol for the item index">
                <Input.TextArea
                  defaultValue={gallerySettings.customDescription || ''}
                  rows="4"
                  onChange={(e) => setGallerySettings({ customDescription: e.target.value })}
                />
              </Form.Item>
              <Form.Item label="Generate Random Description" labelAlign="left">
                <Switch
                  checked={!!gallerySettings.loremDescription}
                  onChange={(e) => setGallerySettings({ loremDescription: e })}
                />
              </Form.Item>
              <Form.Item label="Custom Alt" help="use the # symbol for the item index">
                <Input
                  defaultValue={gallerySettings.customAlt || ''}
                  onChange={(e) => setGallerySettings({ customAlt: e.target.value })}
                />
              </Form.Item>
            </Form>
          </Collapse.Panel>
          <Collapse.Panel header="Styles" key="styles">
            <Form labelCol={{ span: 17 }} wrapperCol={{ span: 3 }}>
              <Form.Item label="Show all Styles" labelAlign="left">
                <Switch
                  checked={!!gallerySettings.showAllStyles}
                  onChange={(e) => setGallerySettings({ showAllStyles: e })}
                />
              </Form.Item>
              <Form.Item label="Responsive Preview" labelAlign="left">
                <Switch
                  checked={!!gallerySettings.responsivePreview}
                  onChange={(e) => {
                    setGallerySettings({ responsivePreview: e });
          ...
          <Collapse.Panel header="Simulators" key="simulators">
            <Form labelCol={{ span: 17 }} wrapperCol={{ span: 3 }}>
              ...
              <Form.Item label="Use Blueprints" labelAlign="left">
                <Switch
                  ...
                  onChange={(e) => setGallerySettings({ useBlueprints: e })}
                />
              </Form.Item>
              <Form.Item label="Use Custom Navigation Panel" labelAlign="left">
                <Switch
                  checked={!!gallerySettings.useCustomNavigationPanel}
                  onChange={(e) => setGallerySettings({ useCustomNavigationPanel: e })}
                />
              </Form.Item>
              <Form.Item label="Use Custom Gallery UI" labelAlign="left">
                <Switch
                  checked={!!gallerySettings.useCustomGalleryUI}
                  onChange={(e) => setGallerySettings({ useCustomGalleryUI: e })}
                />
              </Form.Item>
              <Form.Item label="Use Server Blueprints" labelAlign="left">
                <Switch
                  checked={!!gallerySettings.shouldUseBlueprintsFromServer}
                  onChange={(e) => setGallerySettings({ shouldUseBlueprintsFromServer: e })}
                />
              </Form.Item>
              <Form.Item label="Use Inline Styles" labelAlign="left">
                <Switch
                  checked={!!gallerySettings.useInlineStyles}
                  onChange={(e) => setGallerySettings({ useInlineStyles: e })}
                />
              </Form.Item>
              <Form.Item label="Click to Expand" labelAlign="left">
                <Switch
                  checked={!!gallerySettings.clickToExpand}
                  onChange={(e) => setGallerySettings({ clickToExpand: e })}
                />
              </Form.Item>
            </Form>
            <Form layout="vertical">
              <Form.Item label="View Mode">
                <Select
                  defaultValue={gallerySettings.viewMode || GALLERY_CONSTS.viewMode.SITE}
                  onChange={(val) => setGallerySettings({ viewMode: val })}
                >
                  {Object.entries(GALLERY_CONSTS.viewMode).map(([key, val]) => (
            ...
            <Form layout="vertical">
              <Form.Item label="Navigation Panel Type">
                <Select
                  defaultValue={gallerySettings.navPanelType || 'circles'}
                  onChange={(val) => setGallerySettings({ navPanelType: val })}
                >
                  {['api', 'steps', 'circles'].map((val) => (
            ...
            <Form labelCol={{ span: 17 }} wrapperCol={{ span: 3 }}>
              <Form.Item label="Local Playground" labelAlign="left">
                <Button
                  ...
                  target="_blank"
                  href={`http://localhost:3006/${window.location.search}`}
                />
              </Form.Item>
              <Form.Item label="Master Playground" labelAlign="left">
                <Button
                  ...
                  icon={<ArrowRightOutlined />}
                  target="_blank"
                  href={`https://pro-gallery-master.surge.sh/${window.location.search}`}
                />
              </Form.Item>
              <Form.Item label="Live Playground" labelAlign="left">
                <Button
                  ...
                  icon={<ArrowRightOutlined />}
                  target="_blank"
                  href={`https://pro-gallery.surge.sh/${window.location.search}`}
                />
              </Form.Item>
              {isDev && (
                <Form.Item label="Simulate Local SSR" labelAlign="left">
                  <Button
                    shape="circle"
                    icon={<BugOutlined />}
                    target="_blank"
                    href={`http://localhost:3001/?seed=${Math.floor(
                      Math.random() * 10000
                    )}&allowSSR=true&useCustomNavigationPanel=${
                      gallerySettings.useCustomNavigationPanel
                    }&useCustomGalleryUI=${gallerySettings.useCustomGalleryUI}&useBlueprints=${
                      gallerySettings.useBlueprints
                    }&${getContainerUrlParams(gallerySettings)}&${Object.entries(options)
                      .reduce((arr, [styleParam, value]) => arr.concat(`${styleParam}=${value}`), [])
                      .join('&')}`}
                  />
                </Form.Item>
              )}
          ...
          {isDev && (
            <Collapse.Panel header="ToDos" key="todos">
              {comments.map((comment, idx) => (
                <Alert key={idx} message={comment} type="info" />
              ))}
            </Collapse.Panel>
          )}
        </Collapse>
        ...
        {!utils.isMobile() && !!visible && (
          <>
            ...
            <div className={s.code}>
              <CodePanel />
              <a className={s.report} target="_blank" rel="noreferrer" href="https://github.com/wix/pro-gallery/issues">
                report an issue
              </a>
            </div>
          </>
