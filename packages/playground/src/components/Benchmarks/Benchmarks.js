...
function Benchmarks() {
  ...
  const createLayouts = (layouts) => {
    ...
    console.time(`Created ${numberOfRuns} Layouts with ${numberOfImages} images. It took: `);
    const startTime = Date.now();
    for (let i = 0; i < numberOfRuns; i++) {
      createLayout(layouts[i].layoutParams);
    }
    const duration = Date.now() - startTime;
    console.timeEnd(`Created ${numberOfRuns} Layouts with ${numberOfImages} images. It took: `);
    return duration;
  };
      ...
      setTimeout(() => {
        ...
        setResults(
          <ul>
            ...
            <li>
              Average duration per image is {Math.round((1000 * duration) / numberOfRuns / numberOfImages) / 1000}
              ms
            </li>
          </ul>
        );
  ...
  const renderSlider = (min, max, value, onChange) => (
    <Row>
      <Col span={18}>
        <Slider min={min} max={max} value={value} step={1} onChange={onChange} />
      </Col>
      <Col span={2}>
        <InputNumber
          min={min}
  ...
  return (
    <div className={s.wrapper}>
      <Form layout="vertical">
        ...
        <Form.Item key={'images'} label="Number of Images" labelPlacement={'top'}>
          {renderSlider(0, 1000, numberOfImages, (val) => setNumberOfImages(val))}
        </Form.Item>

        <Button
          block
          onClick={benchmarkType === BENCHMARK_TYPES.LAYOUT ? runLayoutsBenchmarks : runGalleryBenchmarks}
          loading={!!status}
        >
          {status || 'Start'}
        </Button>
