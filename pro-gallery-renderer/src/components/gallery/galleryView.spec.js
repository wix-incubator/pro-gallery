...
describe('Gallery View', () => {
  ...
  describe(' Arrow Keys handler tests ', () => {
    ...
    it('handle up arrow', () => {
      ...
      const result = driver.get.instance().handleArrowKeys(({keyCode: 38, charCode: null, preventDefault() {}, stopPropagation() {}}));
    ...
    it('handle left arrow', () => {
      ...
      const result = driver.get.instance().handleArrowKeys(({keyCode: 37, charCode: null, preventDefault() {}, stopPropagation() {}}));
      expect(stub_document.called).to.be.true;
    ...
    it('handle down arrow', () => {
      ...
      const result = driver.get.instance().handleArrowKeys(({keyCode: 40, charCode: null, preventDefault() {}, stopPropagation() {}}));
      expect(stub_document.called).to.be.true;
    ...
    it('handle right arrow', () => {
      ...
      const stub_utils = sinon.stub(utils, 'setStateAndLog');
      const stub_findNeighborItem = sinon.stub(driver.get.props().galleryStructure, 'findNeighborItem').returns(3);
      const result = driver.get.instance().handleArrowKeys({keyCode: null, charCode: 39, preventDefault() {}, stopPropagation() {}});
      expect(result).to.be.false;
      expect(stub_document.called).to.be.true;
    ...
    it('handle not valid arrow', () => {
      ...
      const stub_document = sinon.stub(document.activeElement, 'getAttribute').returns(7);
      const stub_findNeighborItem = sinon.stub(driver.get.props().galleryStructure, 'findNeighborItem').returns(3);
      const result = driver.get.instance().handleArrowKeys({keyCode: null, charCode: 2, preventDefault() {}, stopPropagation() {}});
