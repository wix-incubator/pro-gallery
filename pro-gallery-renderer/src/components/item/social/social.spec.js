...
describe('Social:', () => {
  ...
  describe('root div, gallery-item-social', () => {
    ...
    it('has classes based on props', () => {
      ...
      expect(driver.find.hook('item-social').hasClass('small-item')).to.equal(true);
      expect(driver.find.hook('item-social').hasClass('short-item')).to.equal(true);
      expect(driver.find.hook('item-social').hasClass('narrow-item')).to.equal(true);
      expect(driver.find.hook('item-social').hasClass('vertical-item')).to.equal(true);
      expect(driver.find.hook('item-social').hasClass('with-arrows')).to.equal(true);
      driver.set.props({
        showShare: false,
        isSmallItem: false,
        isShort: false,
      ...
      expect(driver.find.hook('item-social').hasClass('hidden')).to.equal(false);
      expect(driver.find.hook('item-social').hasClass('small-item')).to.equal(false);
      expect(driver.find.hook('item-social').hasClass('short-item')).to.equal(false);
      expect(driver.find.hook('item-social').hasClass('narrow-item')).to.equal(false);
      expect(driver.find.hook('item-social').hasClass('vertical-item')).to.equal(false);
      expect(driver.find.hook('item-social').hasClass('with-arrows')).to.equal(false);
    });
  });
  describe('social button (getSocialShare)', () => {
    it('should call toggleShare onClick if isSite is true', () => {
      ...
      driver.find.hook('gallery-item-social-button').simulate('click', mockEvent);
    ...
    it('should get a Share component if its a slideshow', () => {
      const stub = sinon.stub(utils, 'isSite').returns(true);
      Object.assign(sampleItemViewProps, {styleParams: {allowSocial: true, isSlideshow: true}});
      driver.mount(ItemView, sampleItemViewProps);
      expect(driver.find.selector('[data-hook="item-social"] > [data-hook="gallery-item-social-button"] > Share').length).to.equal(1);
      driver.set.props({styleParams: {allowSocial: true, isSlideshow: false}});
      expect(driver.find.selector('[data-hook="item-social"] > [data-hook="gallery-item-social-button"] > Share').length).to.equal(0);
      stub.restore();
    });
  });
  describe('love button (getLoveButton)', () => {
    it('should get a LoveButton  if its a slideshow', () => {
      ...
      driver.mount(Social, sampleItemViewProps);
      expect(driver.find.selector('[data-hook="item-social"] > LoveButton').length).to.equal(1);
      driver.set.props({styleParams: {loveButton: false}});
      expect(driver.find.selector('[data-hook="item-social"] > LoveButton').length).to.equal(0);
    });
    it('should toggle slideshow or gallery layout based on props', () => {
      Object.assign(sampleItemViewProps, {styleParams: {allowSocial: true, isSlideshow: true, loveButton: true}});
      driver.mount(ItemView, sampleItemViewProps);
      expect(driver.find.selector(LoveButton).props().layout).to.equal('slideshow');
      driver.set.props({styleParams: {allowSocial: true, isSlideshow: false, loveButton: true}});
      expect(driver.find.selector(LoveButton).props().layout).to.equal('gallery');
    });
  });
  describe('Download button (getDownload)', () => {
    describe('should have a download button only in the right constellation of params -', () => {
      it('allowParams-toggles', () => {
        ...
        Object.assign(sampleItemViewProps, {styleParams: {allowDownload: true}, isDemo: true});
        driver.mount(ItemView, sampleItemViewProps);
        expect(driver.find.hook('item-download').length).to.equal(1); // isSite:false,isiOS:false,allowDownload:true,isDemo:true
        driver.set.props({styleParams: {allowDownload: false}, isDemo: true});
      ...
      it('isSite value doesnt matter while isDemo is false', () => {
        const stubSite = sinon.stub(utils, 'isSite').returns(false);
        const stubiOS = sinon.stub(utils, 'isiOS').returns(false);
        Object.assign(sampleItemViewProps, {styleParams: {allowDownload: true}, isDemo: false});
        driver.mount(ItemView, sampleItemViewProps);
      ...
      it('isDemo value doesnt matter while isSite is false', () => {
        ...
        driver.mount(ItemView, sampleItemViewProps);
        expect(driver.find.hook('item-download').length).to.equal(1); // isSite:false,isiOS:false,allowDownload:true,isDemo:true
        driver.set.props({styleParams: {allowDownload: true}, isDemo: false});
    ...
    it('should create a different onClick functions for text items', () => {
      ...
      Object.assign(sampleItemViewProps, {type: 'text', styleParams: {allowDownload: true}, isDemo: false});
      driver.mount(ItemView, sampleItemViewProps);
