...
class LoveButton extends React.Component {
  ...
  toggleLove(e) {
    ...
    this.setState({
      animate: (this.props.isLoved ? false : true)
    });
  }
  ...
  render() {
    ...
    let loveButtonClassName = [this.buttonClassName(), itemActions.viewClassName(layout, device), this.isLovedClassName(), this.animatedClassName()].join(' ');
    let counterClassName = [this.counterClassName(), itemActions.viewClassName(layout, device)].join(' ');
    let loveCounter = this.props.showCounter && (this.props.count > 0) ? (
        <i
    ...
    let clickAction = utils.getMobileEnabledClick(this.toggleLove);

    return (
        <span data-hook='love-button'
             className={this.containerClassName() + ' show-tooltip '}
             onMouseOver={(e) => {if (this.props.isSettings) {itemActions.showTooltip(e, 'This option is not available in editor')}}}
             onMouseOut={() => {if (this.props.isSettings) {itemActions.hideTooltip()}}}
             {...clickAction}
          >
          {loveButton}
          {loveCounter}
