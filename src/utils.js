...
class Utils {
  ...
  parseGetParam(val, url) {
    ...
    if (!result) {
      ...
      let query = decodeURIComponent(_location.pathname).split('?')[1];
      if (!query) {
        return '';
      }
  ...
  isWixMobile() {
    let _isWixMobile = () => {
      let deviceType = this.parseGetParam('deviceType');
      if (deviceType) {
  ...
  isMobile() {
    let _isMobile = () => {
      let isWixMobile = this.isWixMobile();
      let isUserAgentMobile = this.isUserAgentMobile();
  ...
  isDev() {
    ...
    let ipRegex = /([0-9]{1,3}\.){3}[0-9]{1,3}:[0-9]{1,5}/; //matches 111.222.333.444:9999
